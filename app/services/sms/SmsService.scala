...
trait SmsService
{
  ...
  def parseMobileNumber(mobileNumber: PhoneNumber): Try[String] = mobileNumber match
    {
      case number if number.startsWith(MOBILE_NUMBER_PREFIX) && number.length == 12 => Success(number)
      case number if number.startsWith("4") && number.length == 9 => parseMobileNumber(s"$MOBILE_NUMBER_PREFIX$number")
      case number if number.startsWith("04") && number.length == 10 => parseMobileNumber(number.tail)
